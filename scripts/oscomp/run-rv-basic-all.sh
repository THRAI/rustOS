...
TESTS="brk chdir clone close dup2 dup execve exit fork fstat getcwd getdents getpid getppid gettimeofday mkdir_ mmap mount munmap openat open pipe read sleep times umount uname unlink wait waitpid write yield"

run_basic_group() {
    libc="$1"
    echo "#### OS COMP TEST GROUP START basic-$libc ####"
    cd "/riscv/$libc/basic" || return 1
    ...
    echo "#### OS COMP TEST GROUP END basic-$libc ####"
}

run_basic_group musl || exit 1
run_basic_group glibc || exit 1
